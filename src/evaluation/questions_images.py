...
Given the layout of a {room_type} in {format} and a rendered floorplan image, calculate the Euclidean distance in meters between the centroids of '{obj1}' and '{obj2}'.
...
free_space_question = """
Given the {room_type} layout in {format} and a rendered floorplan image, calculate the total non-occupied (free) floor area in square meters (m²).
...
max_box_question = """
Given the {room_type} layout in {format} and a rendered floorplan image, calculate the area in square meters (m²) of the largest rectangle that can fit inside the room. The rectangle may be rotated at any angle.
...
obstruction_prompt = """
Given the {room_type} layout in {format} and a rendered floorplan image, find all objects that intersect the vector from the centroid of the '{obj1}' to the centroid of the '{obj2}'. Do not include the starting or ending objects themselves.
...
shortest_path = """
Given the {room_type} layout in {format} format below and a rendered floorplan image, determine the shortest valid path that maintains a clearance of {clearance} m from all other objects, starting from the centroid of '{obj1}' and ending at the centroid of '{obj2}'.
...
view_angle = """
Given the {room_type} layout in {format} format below and a rendered floorplan image, compute the smallest absolute angle in degrees (0°–180°) between the vector from the centroid of the '{obj1}' to the centroid of the '{obj2}' and the global north vector (0, 1).
...
placement = """
Given the {room_type} layout in {format} format below and a rendered floorplan image, check if the rectangle object '{object_name}' (Width={object_width}m, Depth={object_depth}m) can fit fully inside the room. The rectangle may be rotated at any angle.
Rectangle must not overlap with other objects and openings apart from floor soft coverings, but may occupy all free space. Ignore passages, door swings, or clearance requirements.
...
repositioning = """
Given the {room_type} layout in {format} format below and a rendered floorplan image, calculate how far the object '{object_to_move}' can be moved in the '{direction}' direction until it touches another object or the boundary of a room.
