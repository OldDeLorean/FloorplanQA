"""
Image-only prompts: no JSON room layout is provided.
The VLM must rely entirely on the rendered floorplan image + coordinate grid.
"""

pair_distance_question = """
Given the rendered floorplan image of a {room_type} (top-down view with metric coordinate grid), calculate the Euclidean distance in meters between the centroids of '{obj1}' and '{obj2}'.

Use the coordinate grid on the image axes to estimate object positions.
...
If the object names or required input data are missing, invalid, or inconsistent, reply with:
*Final answer*: ERROR
...
free_space_question = """
Given the rendered floorplan image of a {room_type} (top-down view with metric coordinate grid), calculate the total non-occupied (free) floor area in square meters (m²).

Use the coordinate grid on the image axes to estimate room boundaries and object dimensions.
...
If the required input data are missing, invalid, or inconsistent, reply with:
*Final answer*: ERROR
...
max_box_question = """
Given the rendered floorplan image of a {room_type} (top-down view with metric coordinate grid), calculate the area in square meters (m²) of the largest rectangle that can fit inside the room. The rectangle may be rotated at any angle.
...
obstruction_prompt = """
Given the rendered floorplan image of a {room_type} (top-down view with metric coordinate grid), find all objects that intersect the vector from the centroid of the '{obj1}' to the centroid of the '{obj2}'. Do not include the starting or ending objects themselves.
...
shortest_path = """
Given the rendered floorplan image of a {room_type} (top-down view with metric coordinate grid), determine the shortest valid path that maintains a clearance of {clearance} m from all other objects, starting from the centroid of '{obj1}' and ending at the centroid of '{obj2}'.

Use the coordinate grid on the image axes to estimate object positions and plan the path.
...
view_angle = """
Given the rendered floorplan image of a {room_type} (top-down view with metric coordinate grid), compute the smallest absolute angle in degrees (0°–180°) between the vector from the centroid of the '{obj1}' to the centroid of the '{obj2}' and the global north vector (0, 1).
...
placement = """
Given the rendered floorplan image of a {room_type} (top-down view with metric coordinate grid), check if the rectangle object '{object_name}' (Width={object_width}m, Depth={object_depth}m) can fit fully inside the room. The rectangle may be rotated at any angle.
Rectangle must not overlap with other objects and openings apart from floor soft coverings, but may occupy all free space. Ignore passages, door swings, or clearance requirements.

Use the coordinate grid on the image axes to estimate available space.
...
repositioning = """
Given the rendered floorplan image of a {room_type} (top-down view with metric coordinate grid), calculate how far the object '{object_to_move}' can be moved in the '{direction}' direction until it touches another object or the boundary of a room.
...
Use the coordinate grid on the image axes to estimate positions and distances.
...
If the object names, direction, or required input data are missing, invalid, or inconsistent, reply with:
*Final answer*: ERROR
